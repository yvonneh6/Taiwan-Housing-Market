...
boxcox_results=boxcox(full.model)
optimal_lambda <-boxcox_results$x[which.max(boxcox_results$y)]

### Transformed Model
trans_model = lm((HousePrice^(optimal_lambda)-1)/optimal_lambda ~ TransactionDate + HouseAge + log(DistanceToMRT) + NumberOfStores + Latitude + Longitude, data = train)
...
ridge_train$HousePrice = (ridge_train$HousePrice^(optimal_lambda)-1)/optimal_lambda
